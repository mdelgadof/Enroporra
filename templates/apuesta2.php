...
<script type='text/javascript'>
	...
	function revisaForm() {
		...
		if ($("#nick").val()=="") {
			alert("Sin tu nick no podemos validar la apuesta. Si no lo recuerdas envíanos un email a <?= $EMAIL_ADMIN ?>");
			$("#nick").focus();
			return false;
		}

		if ($("#id").val()=="" && $("#telefono").val()=="" && $("#email").val()=="") {
			alert("Necesitamos al menos uno de los tres campos (número de apostante, teléfono o email). Si no recuerdas ninguno envíanos un email a <?= $EMAIL_ADMIN ?>");
			$("#email").focus();
			return false;
		}
