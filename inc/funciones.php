...
function porra($id_porrista,$fase=1,$admin=false) {
	...
	$arra=mysql_fetch_array($res);
        ...
        if ($fase>1 && date("Y-m-d H:i:s")<"2012-06-21 20:45:00" && !$admin) {
            if ($arra["arbitro"]!="") "Este jugador ha realizado ya su apuesta para la segunda fase. La verás cuando empiece el primer partido de las elimintatorias :)";
            else echo "Este jugador todavía no ha realizado su apuesta para la segunda fase.";
            return;
        }
	...
	if ($fase>1) echo "<span class='red'><b>Árbitro de la final:</b></span> ".$arra["arbitro"]."<br>";
...
function puntos($id_porrista) {
	...
	if ($id_arbitro==208) {
		...
		$stringDevuelve="<span class='red'>ÁRBITRO DE LA FINAL</span><br>";
		$stringDevuelve.="<img src='".WEB_ROOT."/images/badges/portugal.png' width=16 height=16> <b>PEDRO PROENÇA (PORTUGAL)</b> <span class='green'>5 puntos</span> ";
	...
	if (in_array($id_goleador,array(242,229,209,234,249,224))) {
		...
		$stringDevuelve.="<span class='red'>PICHICHI DE LA EURO (BALOTELLI, CRISTIANO RONALDO, DZAGOEV, FERNANDO TORRES, MANDZUKIC O MARIO GÓMEZ)</span><br>";
	...
	if ($puntos==0) $stringDevuelve="<span class='red'>".$nick."</span> todavía no se ha estrenado, estamos esperando su momento :)";
...
function partido($id_partido) {
	...
	$devuelve.="
		...
		<tr>
			<td align='center' colspan='2'>Resultado más repetido: <span class='red'><b>".$resultado."</b></span></td>
		</tr>";
	}
	else {
		if ($id_partido==63) {
			$devuelve.="
			<tr>
				<td align='center' colspan='2'>El partido por el tercer puesto sólo cuenta en Enroporra a efectos del pichichi</td>
			</tr>";
		}
...
function normalizaNombre($nombre) {
	...
	$nombre=ucwords(strtr(strtolower($nombre), "ÁÉÍÓÚÑÜ", "áéíóúñü"));
...
function clasificacion($tipo="completa") {
	...
	if ($partidos) {
		...
		$devuelve.= "<tr><td colspan='3'></td><td>Pts.</td><td>Apuesta</td><td>&nbsp;&nbsp;&nbsp;&nbsp;Apuesta en próximos partidos</td></tr>";
		foreach ($arrayPorristas as $porrista) {
			...
			$colorFuturaApuesta = (strtolower($nickRegistrado)==strtolower($porrista["nick"])) ? "bgColor='#FFFF00'":"bgColor='$bgColor'";
                        ...
                        if ($porrista["id_arbitro"]>0 && date("Y-m-d H:i:s")<"2012-06-21 20:45:00") {
                            ...
                            if ($partidosSegundaFase==$PARTIDOS_SEGUNDA_FASE) $segundaFaseOK="<span class='green'>2ª Fase OK</span>";
